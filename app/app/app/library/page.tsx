          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
            <thead>
              <tr style={{ background: "var(--paper-deep)", color: "var(--ink-3)", fontSize: 10.5, letterSpacing: "0.05em" }}>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 400 }}>文件名</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 400 }}>状态</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 400 }}>大小</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 400 }}>上传时间</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 400 }}>来源</th>
              </tr>
            </thead>
            <tbody>
              {uploads.map((upload) => (
                <tr key={upload.id} style={{ borderTop: "1px solid var(--paper-line)" }}>
                  ...
                  <td style={{ padding: "12px 14px" }}>
                    <span className={`tz-chip${upload.phase === "ready" ? " is-accent" : ""}`}>
                      {upload.phase === "ready" ? "已就绪" : upload.phase === "failed" ? "失败" : upload.phase}
                    </span>
                  </td>
                  <td style={{ padding: "12px 14px", color: "var(--ink-2)", fontFamily: "var(--mono)" }}>{upload.sizeText}</td>
                  <td style={{ padding: "12px 14px", color: "var(--ink-2)" }}>{upload.createdAtLabel}</td>
                  <td style={{ padding: "12px 14px" }}>
