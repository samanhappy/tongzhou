...
export async function listByTenant(tenantId: string): Promise<Track[]> {
  return getDb()
    .prepare(
      `SELECT * FROM tracks WHERE tenant_id = ? ORDER BY position, created_at DESC`,
    )
    .all<Track>([tenantId]);
}
...
export async function update(
  ...
  await getDb()
    .prepare(
      `UPDATE tracks SET ${fields.join(", ")} WHERE tenant_id = ? AND id = ?`,
    )
    .run(values);
...
export async function recomputeStats(
  ...
  const sums = await db
    ...
    .get<{ sum_dur: number; sum_views: number; total: number }>([
      tenantId,
      trackId,
    ]);
